...
class Entry {
        constructor(urlTemplate, objectTemplate) {
                this.urlTemplate = null;
                this.objectTemplate = null;
                this.urlMatcher = null;
                this.urlComponents = null;
                this.placeholders = null;
                this._compile(urlTemplate, objectTemplate);
        }

        _compile(urlTemplate, objectTemplate) {
        ...
        fromURL(url) {
                ...
                for (const placeholder of this.placeholders) {
                        const value = match[placeholder.urlMatcherGroupIndex];
        ...
        toURL(object) {
                ...
                for (let i = 0; i < this.urlComponents.length; ++i) {
                        ...
                        if (component instanceof Placeholder) {
                                const value = placeholderValues[component.index];
                                url += value;
                        }
                        else {
                                url += component;
